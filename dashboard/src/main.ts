...
const getValue = (id: string): string => getById<HTMLInputElement>(id)?.value ?? ''

const isChecked = (id: string): boolean => Boolean(getById<HTMLInputElement>(id)?.checked)

const closestFromEvent = <T extends HTMLElement = HTMLElement>(event: Event, selector: string) =>
  event.target instanceof Element ? (event.target.closest(selector) as T | null) : null

async function loadData(): Promise<AnyRecord> {
  const res = await fetch('./data.json', { cache: 'no-store' })
  if (!res.ok) throw new Error('Could not load data.json')
  return res.json()
}

function currentDateInDashboardTZ(): string {
...
function renderTotals(totals: AnyRecord): void {
...
function renderWeekHeader(weekHeader: AnyRecord | null): void {
  ...
  const mainNums = String(weekHeader.main_pct || '')
    .split('/')
  ...
  const bars = mainNums
    .map(
      (n) =>
        `<div class="pct-bar"><span style="width:${clampPct(n)}%; background:${barFill(n)}"></span><label>${n}%</label></div>`
    )
    .join('')
...
function renderCycleControl(cycleControl: AnyRecord = {}): void {
  ...
  const options = profiles
    .map((p) => `<option value="${p.code}" data-days="${p.default_days || 7}">${p.name}</option>`)
    .join('')
  const events = (cycleControl.recentEvents || [])
    .slice(0, 5)
    .map(
      (e) =>
        `<li>${e.event_date} · ${e.event_type}${e.deload_code ? ` (${e.deload_code})` : ''}</li>`
    )
    .join('')
...
function renderTodayGlance(
  ...
  const suppText = plannedSupp
    ? `${d.planned_barbell_supp} ${d.planned_supp_sets || ''}x${d.planned_supp_reps || ''}`
    : ''
...
function renderWeeklyCompletion(weekRows: AnyList = [], details: AnyRecord = {}): void {
  let planned = 0
  let done = 0
...
function renderPerformanceKpis(weekRows: AnyList = [], details: AnyRecord = {}): void {
  ...
  const plannedWeek = weekRows.reduce(
    (a, r) =>
      ...
      !!r.main_lift +
      !!r.main_lift +
      (!!r.cardio_plan && r.cardio_plan !== 'OFF') +
      !!r.rings_plan,
    0
  )
  ...
  const volSeries = recentDates.map((d) =>
    (barbellByDate[d] || []).reduce(
      (a, r) => a + Number(r.actual_weight_kg || 0) * Number(r.actual_reps || 0),
      0
    )
  )
  const mainSeries = recentDates.map((d) =>
    (barbellByDate[d] || [])
      .filter((r) => r.category === 'main')
      .reduce((a, r) => a + Number(r.actual_weight_kg || 0) * Number(r.actual_reps || 0), 0)
  )
  const suppSeries = recentDates.map((d) =>
    (barbellByDate[d] || [])
      .filter((r) => r.category === 'supplemental')
      .reduce((a, r) => a + Number(r.actual_weight_kg || 0) * Number(r.actual_reps || 0), 0)
  )
  const cardioSeries = recentDates.map((d) =>
    (cardioByDate[d] || []).reduce((a, r) => a + Number(r.duration_min || 0), 0)
  )
...
function renderEst1RM(rows: AnyList = []): void {
  ...
  const spark = (series = []) => {
    ...
    const pts = vals
      .map(
        (v, i) =>
          `${(i * (w / (vals.length - 1))).toFixed(1)},${(h - pad - ((v - min) / span) * (h - 2 * pad)).toFixed(1)}`
      )
      .join(' ')
  ...
  node.innerHTML = rows
    .map((r) => {
      ...
      if (typeof r.trend_points === 'string') {
        try {
          trend = JSON.parse(r.trend_points) || []
        } catch {
          trend = []
        }
...
function renderCurrentCyclePlan(rows: AnyList = []): void {
  ...
  const summarizeTile = (items) => {
    ...
    const suppLifts = [
      ...new Set(items.filter((x) => x.category === 'supplemental').map((x) => x.lift))
    ]
  ...
  node.innerHTML = weekStarts
    .map((ws, idx) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        cells.push(
          `<article class="cycle-day-tile" data-cycle-date="${date}" tabindex="0"><div class="tile-date">${dow[i]} · ${date}</div><div class="tile-main">${mainTxt}</div><div class="muted">Supp: ${suppTxt}</div></article>`
        )
      }
      return `
...
function renderCardioAnalytics(data: AnyRecord = {}): void {
  ...
  if (recentZ2.length >= 2) {
    ...
    const grid = ticks
      .map((t) => `<line x1="${left}" y1="${t.y}" x2="${w - right}" y2="${t.y}" class="z2-grid"/>`)
      .join('')
    const yLabels = ticks
      .map(
        (t) =>
          `<text x="${left - 6}" y="${t.y + 3}" class="z2-label" text-anchor="end">${t.hr}</text>`
      )
      .join('')
    const circles = pts
      .map(
        (p) =>
          `<g><circle cx="${p.x}" cy="${p.y}" r="2.8" style="opacity:${p.estimated ? '0.65' : '1'}"></circle><title>${p.date}: HR ${p.hr}${p.estimated ? ' (from max HR)' : ''}</title></g>`
      )
      .join('')
    ...
    const effRaw = pts
      .map((p) => {
        const sp = Number(speedByDate.get(p.date))
    ...
    if (effRaw.length >= 2) {
      ...
      const firstE = effRaw[0].eff,
        lastE = effRaw[effRaw.length - 1].eff
  ...
  if (z2Scatter.length >= 2) {
    ...
    const circles = withXY
      .map(
        (p) =>
          `<circle cx="${p.x}" cy="${p.y}" r="3.2" style="fill:#59a8ff;opacity:${p.opacity}"><title>${p.date}: ${p.speed.toFixed(1)} km/h · HR ${p.hr}</title></circle>`
      )
      .join('')
  ...
  if (z2Efficiency.length >= 1) {
    ...
    const vo2Recent = [...vo2Points]
      .slice(-6)
      .filter((p) => Number(p.avg_speed_kmh || p.max_speed_kmh || 0) > 0)
  ...
  const drawMiniSeries = (rows, getY, yFmt = (v) => v) => {
    ...
    const pts = rows.map((r, i) => {
      ...
      return {
        x: left + i * (plotW / (rows.length - 1)),
        y: top + (1 - (v - min) / span) * plotH,
        v,
        d: r.date
      }
    })
  ...
  if (recentVO2.length >= 1) {
    ...
    const rows = recentVO2
      .map((p) => ({
        ...
        workMin: Number(
          p.work_min || (p.protocol === 'VO2_4x4' ? 4 : p.protocol === 'VO2_1min' ? 1 : 0)
        ),
    ...
    const makeProtocolChart = (protocol, pointCls, lineCls, label) => {
      ...
      const polyline =
        withXY.length >= 2
          ? `<polyline points="${withXY.map((p) => `${p.x},${p.y}`).join(' ')}" class="vo2-line ${lineCls}"/>`
          : ''
      const points = withXY
        .map((p, idx) => {
      ...
      const grid = ticks
        .map(
          (t) =>
            `<line x1="${leftPad}" y1="${t.y}" x2="${w - rightPad}" y2="${t.y}" class="z2-grid"/>`
        )
        .join('')
      const yLabels = ticks
        .map(
          (t) =>
            `<text x="${leftPad - 6}" y="${t.y + 3}" class="z2-label" text-anchor="end">${t.hrTick}</text>`
        )
        .join('')

      const effRows = series
        .filter((r) => Number.isFinite(r.speed) && r.speed > 0 && Number.isFinite(r.hr) && r.hr > 0)
        .map((r) => {
...
function renderAuditLog(rows: AnyList = []): void {
...
function renderWeekProgress(rows: AnyList): void {
...
function renderDailyTiles(days: AnyList, details: AnyRecord = {}): void {
  ...
  node.innerHTML = orderedDays
    .map((d) => {
      ...
      const suppDetail = plannedSupp
        ? `${d.planned_barbell_supp} ${d.planned_supp_sets || ''}x${d.planned_supp_reps || ''}`.trim()
        : ''
      ...
      const dow = new Date(`${d.session_date}T00:00:00`).toLocaleDateString(undefined, {
        weekday: 'short'
      })
      return `
...
function summarizePlannedBarbell(plannedRows: AnyList = []): AnyRecord {
...
function summarizeActualBarbell(actualRows: AnyList = []): AnyRecord {
...
function renderNextSessionSuggestion(planned: AnyRecord = {}, actual: AnyRecord = {}): string {
  ...
  const plannedSupp = (planned?.plannedBarbellRows || []).filter(
    (r) => r.category === 'supplemental'
  )
  const actualSupp = barbell.filter((r) => r.category === 'supplemental')
  const substitutedSupplemental =
    plannedSupp.length &&
    actualSupp.length &&
  ...
  if (pain === 'red') {
    ...
    suggestion =
      'Keep the same modified supplemental pattern for one more session, then re-test planned loading.'
...
function renderPlannedVsCompleted(planned: AnyRecord = {}, actual: AnyRecord = {}): string {
  ...
  if (plannedBarbellRows.length && actualBarbellRows.length) {
    ...
    if (pSupp.length && aSupp.length) {
      ...
      if (pLift !== aLift || pSets !== aSets || pReps !== aReps || pWt !== aWt) {
        deltas.push(
          `Substitution detected: planned ${pLift} ${pSets}×${pReps} @ ${pWt} → completed ${aLift} ${aSets}×${aReps} @ ${aWt}`
        )
      }
    }
  }
...
function renderBarbellDetails(rows: AnyList = [], planned: AnyRecord = {}): string {
  const pain = planned?.pain_level || 'green'
  if (!rows.length) {
    ...
    const adjust =
      pain === 'yellow'
        ...
        : pain === 'red'
          ? `<p><strong>Auto-adjust (RED):</strong> skip heavy barbell today.</p>`
          : ''
  ...
  if (suppRows.length) {
    ...
    for (const [lift, arr] of byLift.entries()) {
      ...
      const repsText =
        repsSet.length === 1 ? repsSet[0] : arr.map((x) => x.actual_reps ?? '-').join('/')
      const weightText = weightSet.length === 1 ? ` @ ${weightSet[0]} kg` : ''
      const noteText = noteSet.length ? `<div class="detail-note">${noteSet.join(' · ')}</div>` : ''

      items.push(
        `<li><strong>${lift}</strong> · supplemental: ${setCount}×${repsText}${weightText}${noteText}</li>`
      )
    }
...
function renderCardioDetails(rows: AnyList = [], planned: AnyRecord = {}): string {
  const pain = planned?.pain_level || 'green'
  if (!rows.length) {
    const p = planned?.plannedCardio || null
    if (!p || !p.session_type || p.session_type === 'OFF')
      return '<p class="muted">Not expected today (rest / off day).</p>'
    ...
    const adjust =
      pain === 'yellow'
        ...
        : pain === 'red'
          ? `<p><strong>Auto-adjust (RED):</strong> recovery walk only.</p>`
          : ''
  ...
  if (intervals.length) {
    ...
    const allSame =
      workSet.length === 1 && easySet.length === 1 && speedSet.length <= 1 && hrSet.length <= 1
...
function renderRingsDetails(rows: AnyList = [], planned: AnyRecord = {}): string {
  const pain = planned?.pain_level || 'green'
  if (!rows.length) {
    ...
    const list = pRows
      ...
      .map(
        (r) =>
          `<li>${r.item_no}. ${r.exercise} · ${r.sets_text}×${r.reps_or_time}${r.tempo ? ` @ ${r.tempo}` : ''}${r.rest_text ? ` · rest ${r.rest_text}` : ''}</li>`
      )
      .join('')
    ...
    const adjust =
      pain === 'red'
        ...
        : pain === 'yellow'
          ? `<p><strong>Auto-adjust (YELLOW):</strong> keep technique easy, stop if pain rises.</p>`
          : ''
  ...
  const list = rows
    .filter((r) => r.item_no != null)
    .map((r) => `<li>${r.item_no}. ${r.exercise}${r.result_text ? ` · ${r.result_text}` : ''}</li>`)
    .join('')
  return `
    <p><strong>Template ${tpl}</strong></p>
...
function bindStatusPicker(renderDashboardFn: () => Promise<void>): void {
...
function bindDetailClicks(
  ...
  function openForDate(date) {
    ...
    const mainPrescription = mainRows.length
      ? mainRows.map((r) => `${r.planned_weight_kg}×${r.prescribed_reps}`).join(' · ')
      : '—'
    const suppRows = (planned.plannedBarbellRows || []).filter((r) => r.category === 'supplemental')
    const supp = suppRows[0] || null
    const suppPrescription = suppRows.length
      ? `${suppRows.length}×${supp?.prescribed_reps ?? '-'} @ ${supp?.planned_weight_kg ?? '-'} kg`
      : '—'
    ...
    const ringsPlanText =
      ringsRows
        ...
        .map(
          (r) =>
            `[${r.template_code}] ${r.item_no}. ${r.exercise} ${r.sets_text || ''}x${r.reps_or_time || ''}`
        )
  ...
  document.querySelectorAll('.tile, .week-row').forEach((el) => {
    const open = (evt) => {
      ...
      openForDate((el as HTMLElement).dataset.date || '')
    }
    el.addEventListener('click', open)
    el.addEventListener('keydown', (e: KeyboardEvent) => {
      if (e.key === 'Enter' || e.key === ' ') {
...
function initUploadBox() {
  const wire = ({ kind, inputId, btnId, statusId, boxId }) => {
    ...
    btn.addEventListener('click', async () => upload((input as HTMLInputElement).files?.[0]))
...
function initTabs() {
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener('click', () => activate((btn as HTMLElement).dataset.tab || 'overview'))
  })

  const fromHash = (window.location.hash || '').replace('#tab-', '')
  const initialTab = tabButtons.some((b) => (b as HTMLElement).dataset.tab === fromHash)
    ? fromHash
  ...
  try {
    ...
    window.logSessionAction = async (action) => {
      ...
      if (action === 'finish_session') {
        const mainW = parseFloat(getValue('mainWeightInput'))
        const mainR = parseInt(getValue('mainRepsInput'), 10)
        const mainRpe = parseFloat(getValue('mainRpeInput'))

        const suppCompleted = isChecked('suppCompletedInput')
        const suppModified = isChecked('suppModifiedInput')
        const suppW = parseFloat(getValue('suppWeightInput'))
        const suppSets = parseInt(getValue('suppSetsInput'), 10)

        const cDur = parseInt(getValue('cardioDurationInput'), 10)
        const cHr = parseInt(getValue('cardioAvgHrInput'), 10)
        const cSpeed = parseFloat(getValue('cardioSpeedInput'))
        const cWorkMin = parseFloat(getValue('cardioWorkMinInput'))
        const cRestMin = parseFloat(getValue('cardioRestMinInput'))
        ...
        if (Number.isFinite(mainW) && Number.isFinite(mainR) && mainW > 0 && mainR > 0) {
          await postLogAction({
            action: 'main_done',
            date,
            plannedBarbellRows: (planned.plannedBarbellRows || []).map((r) =>
              r.category === 'main'
                ? {
                    ...r,
                    planned_weight_kg: mainW,
                    prescribed_reps: mainR,
                    note: Number.isFinite(mainRpe) ? `RPE ${mainRpe}` : r.note
                  }
                : r
            ),
            plannedCardio: planned.plannedCardio || null
        ...
        if (suppModified && Number.isFinite(suppW) && suppW > 0) {
          const reps =
            (planned.plannedBarbellRows || []).find((r) => r.category === 'supplemental')
              ?.prescribed_reps || 5
        ...
        if (Number.isFinite(mainW) && Number.isFinite(mainR) && mainW > 0 && mainR > 0) {
          ...
          const prevMain = (
            (window.__dashboardData?.details?.barbellByDate || {})[date] || []
          ).filter((r: AnyRecord) => r.category === 'main')
      ...
      if (action === 'cardio_done' || action === 'z2_fixed_hr_test') {
        const hrTxt = getValue('cardioAvgHrInput').trim()
        const durInput = parseInt(getValue('cardioDurationInput'), 10)
        const speedInput = parseFloat(getValue('cardioSpeedInput'))
        const workMinInput = parseFloat(getValue('cardioWorkMinInput'))
        const restMinInput = parseFloat(getValue('cardioRestMinInput'))
        ...
        payload.plannedCardio = {
          ...(planned.plannedCardio || {}),
          duration_min:
            Number.isFinite(durInput) && durInput > 0
              ? durInput
      ...
      if (action === 'main_done') {
        ...
        if (Number.isFinite(mainW) && mainW > 0 && Number.isFinite(mainR) && mainR > 0) {
          payload.plannedBarbellRows = (planned.plannedBarbellRows || []).map((r) =>
            r.category === 'main' ? { ...r, planned_weight_kg: mainW, prescribed_reps: mainR } : r
          )
        }
      }
    ...
    document.addEventListener('click', (e) => {
      const btn = closestFromEvent(e, '#startSessionBtn')
      if (!btn) return
      const today = currentDateInDashboardTZ()
      if (window.__openDetailForDate) window.__openDetailForDate(today)
    })

    const refreshBtn = getById<HTMLButtonElement>('refreshBtn')
    const refreshHealthBtn = getById<HTMLButtonElement>('refreshHealthBtn')
    ...
    document.addEventListener('click', async (e) => {
      const tmDeltaBtn = closestFromEvent<HTMLButtonElement>(e, '[data-tm-delta]')
      if (tmDeltaBtn) {
      ...
      const tmSetBtn = closestFromEvent<HTMLButtonElement>(e, '[data-tm-set]')
      if (tmSetBtn) {
        const lift = tmSetBtn.getAttribute('data-tm-set')
        const v = parseFloat(getValue(`tmSet-${lift}`))
      ...
      const startBtn = closestFromEvent<HTMLButtonElement>(e, '#startCycleBtn')
      if (startBtn) {
      ...
      const deloadBtn = closestFromEvent<HTMLButtonElement>(e, '#applyDeloadBtn')
      if (deloadBtn) {
        const deloadCode = getValue('deloadTypeInput')
        const startDate = getValue('deloadStartInput')
        const durationDays = parseInt(getValue('deloadDaysInput'), 10) || 7
        try {
