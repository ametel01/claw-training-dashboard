...
import { allRows, getRow, openDatabase, runStatement } from '../lib/db'
import { todayIsoLocal } from '../lib/time'

const MAIN_PCTS: Record<number, number[]> = {
  1: [0.65, 0.75, 0.85],
  2: [0.7, 0.8, 0.9],
  3: [0.75, 0.85, 0.95]
}

const RINGS_TEMPLATES: Record<string, string[]> = {
  A: [
...
type Prescription = {
  ...
  supp_lift: string
  supp_lift_id: number
  main_sets: Array<[number, number, number]>
  supp_sets: number
  supp_reps: number
  supp_weight: number
  fsl_pct: number
...
function optionalNumber(flags: Flags, key: string): number | undefined {
  const value = flags[key]
...
function hasFlag(flags: Flags, key: string): boolean {
  return flags[key] === true
}

function parseDate(value?: string): string {
  if (!value) return todayIsoLocal()
...
function roundToInc(value: number, increment: number): number {
...
function ensureRingsSchema(db: DatabaseSync): void {
  db.exec(`
...
function getConfig(db: DatabaseSync, key: string, fallback = ''): string {
  const row = getRow<{ value: string }>(db, 'SELECT value FROM config WHERE key = ?', key)
  return row?.value ?? fallback
}

function getBlockForDate(db: DatabaseSync, iso: string) {
  return getRow<Record<string, string | number>>(
    db,
    `
...
function getTrainingDay(db: DatabaseSync, iso: string) {
...
function getTm(db: DatabaseSync, liftId: number, iso: string): number {
  const row = getRow<{ tm_kg: number }>(
    db,
    `
      SELECT tm_kg
...
function getWeekInBlock(blockStart: string, iso: string): number {
  return Math.floor((isoDateToUtc(iso) - isoDateToUtc(blockStart)) / (86_400_000 * 7)) + 1
}

function getPrescription(db: DatabaseSync, iso: string): Prescription | null {
  const trainingDay = getTrainingDay(db, iso)
  const block = getBlockForDate(db, iso)
  if (!trainingDay || !block) return null
...
function parseSetTriplets(value?: string): Array<[number, number]> {
  if (!value) return []
  return value
    .split(',')
    .map((part) => part.trim())
    ...
    .map((part) => {
      ...
      if (!match) throw new Error(`invalid set format: ${part}`)
      return [Number(match[1]), Number(match[2])] as [number, number]
    })
}

function parseRepList(value?: string): number[] {
  if (!value) return []
  return value
    .split(',')
    .map((part) => part.trim())
...
function ensureSession(
  ...
  runStatement(
    ...
    Number(block.id),
    weekInBlock,
    Number(trainingDay.id),
    bodyweight ?? null,
    readiness ?? null,
    notes ?? null
  )

  const row = getRow<{ id: number }>(db, 'SELECT id FROM barbell_sessions WHERE session_date = ?', iso)
  if (!row) throw new Error('Could not find created session')
  return row.id
}

function nextRingsTemplate(db: DatabaseSync): string {
  const row = getRow<{ template: string }>(
    db,
...
function handleToday(flags: Flags): void {
  ...
  console.log(
    `Block: ${prescription.block_type} ${prescription.block_no} | Week in block: ${prescription.week_in_block} (Wave week ${prescription.week_wave})`
  )
  ...
  console.log(
    `  ${prescription.supp_sets} x ${prescription.supp_reps} @ ${prescription.supp_weight.toFixed(1)} kg (${Math.round(prescription.fsl_pct * 100)}% FSL)`
  )
}

function handleLog(flags: Flags): void {
  const iso = parseDate(optionalString(flags, 'date'))
  const db = getDb()
  const prescription = getPrescription(db, iso)
  if (!prescription) throw new Error('Not a configured training day')
  ...
  const effectiveMainReps = mainDone.length
    ? []
    : mainRepsOnly.length
      ? mainRepsOnly
      : prescription.main_sets.map((item) => item[1])

  const sessionId = ensureSession(
    db,
    iso,
    optionalString(flags, 'notes'),
    optionalNumber(flags, 'bodyweight'),
    optionalNumber(flags, 'readiness')
  )
  ...
  try {
    runStatement(
      db,
      "DELETE FROM barbell_set_logs WHERE session_id = ? AND category IN ('main','supplemental')",
      sessionId
    )

    prescription.main_sets.forEach(([pct, reps, prescribedWeight], index) => {
      const [actualWeight, actualReps] = mainDone.length
        ? mainDone[index]
        : [prescribedWeight, effectiveMainReps[index]]
      ...
      runStatement(
        ...
        `,
        sessionId,
        prescription.main_lift_id,
        index + 1,
        pct,
        reps,
        actualWeight,
        actualReps,
        optionalNumber(flags, 'rpe') ?? null
      )
    })
    ...
    if (suppDone.length) {
      suppDone.forEach(([actualWeight, actualReps], index) => {
        runStatement(
          ...
          `,
          sessionId,
          prescription.supp_lift_id,
          index + 1,
          prescription.fsl_pct,
          prescription.supp_reps,
          actualWeight,
          actualReps
        )
      })
...
function handleRingsToday(flags: Flags): void {
  const iso = parseDate(optionalString(flags, 'date'))
  const db = getDb()
...
function handleRingsLog(flags: Flags): void {
  const iso = parseDate(optionalString(flags, 'date'))
  const db = getDb()
  const template = (optionalString(flags, 'template') ?? nextRingsTemplate(db)).toUpperCase()
  if (!RINGS_TEMPLATES[template]) {
  ...
  runStatement(
    ...
    template,
    completed ? 1 : 0,
    optionalString(flags, 'notes') ?? null
  )
  ...
  if (!session) throw new Error('Could not find rings session')
  ...
  try {
    ...
    RINGS_TEMPLATES[template].forEach((exercise, index) => {
      runStatement(
        ...
        index + 1,
        exercise,
        completed ? 'completed as prescribed' : 'custom/result not specified',
        completed ? 1 : 0
      )
    })
    ...
    if (missed) {
      runStatement(
        db,
        'INSERT INTO rings_logs(session_id, item_no, exercise, result_text, completed) VALUES(?, 99, ?, ?, 0)',
        session.id,
        'Missed/adjustments',
        missed
      )
...
function handleCardioToday(flags: Flags): void {
  const iso = parseDate(optionalString(flags, 'date'))
  const day = new Date(`${iso}T00:00:00Z`)
  const weekday = ((day.getUTCDay() + 6) % 7) + 1
...
function handleCardioLog(flags: Flags): void {
  const iso = parseDate(optionalString(flags, 'date'))
  ...
  runStatement(
    ...
    iso,
    protocol,
    optionalNumber(flags, 'duration') ?? null,
    optionalNumber(flags, 'avg-hr') ?? null,
    maxHr ?? null,
    z2CapRespected,
    optionalString(flags, 'notes') ?? null
  )
...
function handleCardioProgressAdd(flags: Flags): void {
  const iso = parseDate(optionalString(flags, 'date'))
  const db = getDb()
  runStatement(
    db,
    ...
    `,
    iso,
    Number(requireString(flags, 'duration')),
    Number(requireString(flags, 'speed')),
    Number(requireString(flags, 'incline')),
    Number(requireString(flags, 'final-bpm')),
    optionalString(flags, 'notes') ?? null
  )
  console.log(`Logged cardio progress test for ${iso}.`)
}

function handleCardioProgressShow(): void {
  const db = getDb()
  const rows = allRows<{
    test_date: string
    duration_min: number
  ...
  rows.forEach((row) => {
    ...
    console.log(
      `  ${row.test_date}: ${row.duration_min}min @ ${Number(row.speed_kmh).toFixed(1)} km/h, ${Number(row.incline_pct).toFixed(1)}% incline, final ${row.final_bpm} bpm${note}`
    )
  })
}
